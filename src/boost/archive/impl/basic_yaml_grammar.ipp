...
bool basic_yaml_grammar<CharType>::my_parse(
    typename basic_yaml_grammar<CharType>::IStream & is,
    const rule_t & rule_,
    CharType delimiter
) const {
    if(is.fail()){
    ...
    CharType val;
    do{
        typename basic_yaml_grammar<CharType>::IStream::int_type
            result = is.get();
        if(is.fail())
            return false;
...
template<class CharType>
bool basic_yaml_grammar<CharType>::parse_end_tag(IStream & is) const {
...
basic_yaml_grammar<CharType>::basic_yaml_grammar(){
    ...
    STag =
        !S
        >> '<'
        >> Name  [yaml::assign_object(rv.object_name)]
        >> AttributeList
        >> !S
        >> '>'
    ;
    ...
    CharRef1 = 
        str_p(L"&#") >> uint_p [yaml::append_char<StringType>(rv.contents)] >> L';'
    ;
    CharRef2 =
        str_p(L"&#x") >> hex_p [yaml::append_char<StringType>(rv.contents)] >> L';'
    ;
    ...
    AposRef = str_p(L"&apos;")[yaml::append_lit<StringType, L'\''>(rv.contents)];
    ...
    ClassIDAttribute = 
        str_p(BOOST_ARCHIVE_YAML_CLASS_ID()) >> NameTail
        >> Eq 
        >> L'"'
        >> int_p [yaml::assign_object(rv.class_id)]
        >> L'"'
      ;

    ObjectIDAttribute = (
        str_p(BOOST_ARCHIVE_YAML_OBJECT_ID()) 
        | 
        str_p(BOOST_ARCHIVE_YAML_OBJECT_REFERENCE()) 
        )
        >> NameTail
        >> Eq 
        >> L'"'
    ...
    AmpName = str_p(L"&amp;")[yaml::append_lit<StringType, L'&'>(rv.class_name)];
    ...
    ClassNameChar = 
        ...
        | LTName
        | GTName
        | (anychar_p - chset_p(L"\"")) [yaml::append_char<StringType>(rv.class_name)]
    ;
    ...
    ClassNameAttribute = 
        str_p(BOOST_ARCHIVE_YAML_CLASS_NAME()) 
        >> Eq 
        >> L'"'
        >> ClassName
        >> L'"'
    ...
    TrackingAttribute = 
        str_p(BOOST_ARCHIVE_YAML_TRACKING())
        >> Eq
        >> L'"'
        >> uint_p [yaml::assign_level(rv.tracking_level)]
        >> L'"'
    ;

    VersionAttribute = 
        str_p(BOOST_ARCHIVE_YAML_VERSION())
        >> Eq
        >> L'"'
        >> uint_p [yaml::assign_object(rv.version)]
        >> L'"'
    ;
    ...
    SignatureAttribute = 
        ...
        >> Eq 
        >> L'"'
        >> Name [yaml::assign_object(rv.class_name)]
        >> L'"'
    ;
...
template<class CharType>
bool basic_yaml_grammar<CharType>::windup(IStream & is) {
    return my_parse(is, ETag);
}

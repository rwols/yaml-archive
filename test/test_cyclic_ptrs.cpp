...
int test1()
{
    const char* testfile = "cyclic_ptrs1.yml";
    ...
    }
    {
        ...
        ia >> boost::serialization::make_nvp("j1", j2);
    }
    BOOST_CHECK(j1 == j2);
    // std::remove(testfile);
    return EXIT_SUCCESS;
...
int test2()
{
    const char* testfile = "cyclic_ptrs2.yml";
    BOOST_REQUIRE(NULL != testfile);
...
int test3()
{
    const char* testfile = "cyclic_ptrs3.yml";
    BOOST_REQUIRE(NULL != testfile);
        ...
        boost::archive::archive_exception::no_exception);
    {
        ...
        BOOST_CATCH(boost::archive::archive_exception ae) { exception = ae; }
        BOOST_CATCH_END
